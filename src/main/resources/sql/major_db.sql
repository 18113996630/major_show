...
CREATE TABLE `log`  (
                        ...
                        PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1100 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`localhost` SQL SECURITY DEFINER VIEW `vw_video_up` AS select `x`.`upName` AS `upName`,`x`.`count` AS `count`,`x`.`url` AS `url`,`y`.`titles` AS `titles` from (((select `a`.`upName` AS `upName`,`a`.`count` AS `count`,`b`.`up_page` AS `url` from (((select `t`.`up_name` AS `upName`,count(1) AS `count` from `major_db`.`video` `t` group by `t`.`up_name` order by count(`t`.`up_name`) desc)) `a` left join (select distinct `v`.`up_name` AS `up_name`,`v`.`up_page` AS `up_page`,`v`.`face_name` AS `face_name` from `major_db`.`video` `v`) `b` on((`a`.`upName` = `b`.`up_name`))))) `x` left join (select `t`.`up_name` AS `up_name`,group_concat(`t`.`title` separator ',') AS `titles` from `major_db`.`video` `t` group by `t`.`up_name`) `y` on((`x`.`upName` = `y`.`up_name`)));
